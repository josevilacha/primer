...
sys.path.append("/home/jose/Documents/primer/primer/blender_scripts")
sys.path.append("/home/jose/Documents/primer/primer/blender_scripts/tools")
...
sys.path.append(
    "C:\\Users\\justi\\Documents\\CodeProjects\\Primer\\blender_scripts\\video_scenes"
)
...
def initialize_blender(
    total_duration=DEFAULT_SCENE_DURATION, clear_blender=True, vertical=False
):
    ...
    # Set to 60 fps
    # FIXME:  bpy.ops.script.execute_preset(
    ...
    lamp_height = 35
    bpy.ops.object.light_add(type=LAMP_TYPE, location=(0, 0, lamp_height))
    light = bpy.context.object
    light.parent = lamp_parent
...
def marketing():
    ...
    y_blob.blob_wave(start_time=0, duration=12)
...
def draw_scenes_from_file(script_file, clear=True):
    ...
    frame = 0
    for scene in scenes:
        execute_and_time(scene[0], scene[1].play())  # This is just a string
...
def make_blob_with_actions_for_unity():
    ...
    blobj.hold_object(start_time=0, end_time=2)
...
def test():
        ...
        for food_count in food_counts_to_try:
            ...
            for j in range(num_samples):
                ...
                for k in range(num_creatures):
                    if k == num_creatures - 1:  #% 2 == 0:
                        cre = hawk_dove.Creature(fight_chance=1)
                    else:
                        cre = hawk_dove.Creature(fight_chance=0)
                ...
                world = (
                    hawk_dove_basic.World()
                )  # food_count = food_count, initial_creatures = initial_creatures)
                ...
                for day in world.calendar:
                    ...
                    if table_type == "avgs_with_food_count_and_time":
                        ...
                        if day.date + 1 in nums_days_to_try:
                            sample_results = [x.fight_chance for x in day.creatures]
            ...
            if table_type == "avgs_with_food_count_and_time":
                ...
                for i in range(len(nums_days_to_try)):
                    ...
                    results_str += "{0:.2f}".format(
                        round(statistics.mean(samples_at_day), 2)
                    )
                    if len(samples_at_day) > 1:
                        results_str += " +/- " + "{0:.2f}".format(
                            round(statistics.stdev(samples_at_day), 2)
                        )
                ...
                for sample in samples:
                    ...
                    for i in range(quantile_count + 1):  # 1 to catch 100%
                        ...
                        for cre_fight_chance in sample:  # Good naming! ...
                            if (
                                cre_fight_chance < (i + 1) / quantile_count
                                and cre_fight_chance >= i / quantile_count
                            ):
                                num_in_quantile += 1
    ...
    def animation_test():
        ...
        drawn_world = drawn_contest_world.DrawnWorld(sim=world, loud=True)
        ...
        drawn_world.animate_days(
            start_time=2, first_animated_day=0, last_animated_day=5
        )
    ...
    def walk_to_test():
        bleb = blobject.Blobject(
            scale=4, location=[-8, 0, 0], rotation_euler=[math.pi / 2, 0, 0]
        )
    ...
    graph = graph_bobject.GraphBobject(func0, func1, centered=True)
    ...
    graph.morph_curve(
        1, start_time=3
    )  # Change curve to different function. First arg is destination function index
    graph.change_window(  # Animate change of window with same function.
        new_x_range=[0, 15], new_y_range=[0, 20], new_tick_step=[5, 5], start_time=5
    )
