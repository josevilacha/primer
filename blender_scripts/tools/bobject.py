...
class Bobject(object):
    ...
    def __init__(self, *subbobjects, **kwargs):
        ...
        if isinstance(self.intrinsic_scale, int) or isinstance(
            self.intrinsic_scale, float
        ):
    ...
    def add_to_blender(
        ...
        if appear_time != None:
            if appear_frame != None:
                raise Warning(
                    "You defined both appear frame and appear time. "
                    + "Just do one, ya dick."
                )
        ...
        if self.added_to_blender == False:
            ...
            if unhide == True:
                ...
                main_obj.keyframe_insert(
                    data_path="hide_viewport", frame=appear_frame - 1
                )
                main_obj.keyframe_insert(
                    data_path="hide_render", frame=appear_frame - 1
                )
                main_obj.hide_viewport = False
                main_obj.keyframe_insert(data_path="hide_viewport", frame=appear_frame)

                for obj in self.objects:
                    hide_self_and_descendants(
                        obj, keyframes=True, frame=appear_frame - 1
                    )
                    hide_self_and_descendants(
                        obj,
            ...
            if animate == False:
                ...
                main_obj.scale = self.intrinsic_scale
                main_obj.keyframe_insert(
                    data_path="scale", frame=scale_up_frame + duration
                )
        ...
        self.add_subbobjects(
            appear_frame=appear_frame, subbobject_timing=subbobject_timing
        )
    ...
    def add_subbobjects(
        ...
        followers = [
            x
            for x in self.subbobjects
            if x.appear_with_super == True and x.added_to_blender == False
        ]
        ...
        if subbobject_timing == "start":
            ...
            raise Warning(
                "So many types are accepted for subbobject_timing, "
    ...
    def disappear(
        ...
        main_obj.hide_viewport = True
        main_obj.hide_render = True
        main_obj.keyframe_insert(data_path="hide_viewport", frame=disappear_frame)
        ...
        if animate == True:
            # scale_down_frame = disappear_frame + OBJECT_APPEARANCE_TIME Makes the
            # scale-down happen after the hiding if we don't want the bobject to animate out
            scale_down_frame = disappear_frame

            # Uses current scale. This assumes keyframes have been added chronologically, so
            # the previous keyframe agrees with the current object scale.
            main_obj.keyframe_insert(
                data_path="scale", frame=scale_down_frame - duration_frames
            )
            main_obj.scale = [0, 0, 0]
    ...
    def move_to(
        ...
        if end_time != None:
            if end_frame != None:
                raise Warning(
                    "You defined both end frame and end time. "
                    + "Just do one, ya dick."
                )
    ...
    def pulse(
        ...
        if start_time != None:
            if start_frame != None:
                raise Warning(
                    "You defined both start_frame and start_time. "
                    + "Just do one, ya dick."
                )
        ...
        if duration_time != None:
            if duration != None:
                raise Warning(
                    "You defined duration by both frames and time. "
                    + "Just do one, ya dick."
                )
        ...
        obj.keyframe_insert(
            data_path="scale", frame=start_frame + duration - decay_frames
        )
        obj.scale /= factor
    ...
    def color_shift(
        ...
        if start_time != None:
            if start_frame != None:
                raise Warning(
                    "You defined both frame and time. " + "Just do one, ya dick."
                )
            start_frame = int(start_time * FRAME_RATE)
        ...
        if color_gradient == None:
            ...
            color_field.keyframe_insert(
                data_path="default_value", frame=start_frame + shift_time
            )

            # Viewport color
            mat_copy.keyframe_insert(data_path="diffuse_color", frame=start_frame)
            mat_copy.diffuse_color = color[:3]
            mat_copy.keyframe_insert(
                data_path="diffuse_color", frame=start_frame + shift_time
            )

            if duration != None:
                # If duration is finite, return to original color
                color_field.keyframe_insert(
                    data_path="default_value", frame=start_frame + duration - shift_time
                )
                ...
                mat_copy.keyframe_insert(
                    data_path="diffuse_color", frame=start_frame + duration - shift_time
                )
                mat_copy.diffuse_color = original_color[:3]
                mat_copy.keyframe_insert(
                    data_path="diffuse_color", frame=start_frame + duration
                )
    ...
    def wobble(
        ...
        self.ref_obj.keyframe_insert(
            data_path="rotation_euler", frame=start_time * FRAME_RATE
        )
        time += inc
        self.ref_obj.rotation_euler[axis] += max_angle_rad
        self.ref_obj.keyframe_insert(
            data_path="rotation_euler", frame=time * FRAME_RATE
        )
    ...
    def de_explode(self, start_time=0, duration=1):
...
def de_explode_children(obj, start_frame, duration_frames):
    for child in obj.children:
        ...
        child.keyframe_insert(data_path="location", frame=start_frame)
        child.location = final_loc
        child.keyframe_insert(
            data_path="location", frame=start_frame + duration_frames * 6
        )

        final_scale = deepcopy(child.scale)
        child.scale = [0, 0, 0]
        child.keyframe_insert(data_path="scale", frame=start_frame)
        if "Cylinder" in child.name:
            ...
            child.keyframe_insert(
                data_path="scale", frame=start_frame + duration_frames * 16
            )
        ...
        if child.scale[0] > 0.5:
            de_explode_children(
                child, start_frame + duration_frames * 5, duration_frames
            )
        else:
            de_explode_children(child, start_frame + duration_frames, duration_frames)
...
class MeshMorphBobject(Bobject):
    ...
    def add_to_blender(self, **kwargs):
        ...
        self.series[0].add_to_blender(
            animate=False, appear_frame=kwargs["appear_frame"]
        )
    ...
    def add_subbobject_to_series(self, subbobject):
        ...
        if not isinstance(subbobject, Bobject):
            raise Warning(
                "Subbobjects in series should be bobjects, else Justin gets confused."
            )
    ...
    def morph_bobject(
        ...
        self.series[initial_index].disappear(
            disappear_frame=start_frame + 1, animate=False
        )
        self.series[final_index].add_to_blender(appear_frame=end_frame, animate=False)
        ...
        if RENDER_QUALITY == "medium" or RENDER_QUALITY == "high":
            ...
            for first in initial_meshes:
                for last in final_meshes:
                    self.morph_mesh_to_mesh(
                        first, last, start_frame, end_frame, dissolve_time=dissolve_time
                    )
    ...
    def key_particles(
        self, start, end, start_frame, end_frame, backward=False, dissolve_time=0
    ):
        # Once to shoot particles out of the original object
        self.add_morph_particles(
        ...
        stngs.keyframe_insert(
            data_path="particle_size", frame=start_frame + PARTICLE_APPEARANCE_TIME
        )
        stngs.particle_size = 0
        ...
        stngs.particle_size = MORPH_PARTICLE_SIZE
        stngs.keyframe_insert(
            data_path="particle_size", frame=end_frame - PARTICLE_APPEARANCE_TIME
        )
        stngs.particle_size = 0
        stngs.keyframe_insert(data_path="particle_size", frame=end_frame)
...
class RainBobject(Bobject):
    ...
    def make_it_rain(self):
        ...
        dup_bobj = import_object(
            "goodicosphere", "primitives", location=(1000, 0, 0), name="raindrop model"
        )
