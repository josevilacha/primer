...
class TexBobject(SVGBobject):
    """Class for TexBobjects.


    Attributes
    ----------
    paths: list(str)
        List of the paths containing the SVGs with rendered latex.
    ...
    def morph_figure(
        ...
        if (
            isinstance(self.superbobject, tex_complex.TexComplex)
            and "sim_counter" not in self.superbobject.name
            and arrange_super == True
        ):
            self.superbobject.arrange_tex_bobjects(
                start_frame=start_frame, end_frame=start_frame + duration
            )
    ...
    def get_file_paths(self, expressions):
        """Get file paths to the SVGs corresponding to each expression.

        Parameter
        ---------
        expressions: list(str)
            List containing the expressions to be rendered in Tex.
        """
        ...
        for expr in expressions:
            if expr == None:
                ...
                self.paths.append(tex_to_svg_file(expr, template, self.typeface))

    # def decompose_expression(self, expression):
    #
    #     # for char in expression:
    #     # cursor_pos = 0
    #     # while True:
    #     #     if expression[cursor_pos] == "\\":
    #     #         pass
    #     #     else:
    #     #         expression_to_render = expression[cursor_pos]
    #     #         cursor_pos += 1
    #     #     yield expression_to_render
...
def tex_to_svg_file(expression, template_tex_file, typeface):
    print(expression)
    path = os.path.join(TEX_DIR, tex_title(expression, typeface)) + ".svg"
...
def generate_tex_file(expression, template_tex_file, typeface):
    result = os.path.join(TEX_DIR, tex_title(expression, typeface)) + ".tex"
