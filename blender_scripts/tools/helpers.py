...
def save_sim_result(sim, filename, filename_seed, type="SIM"):
    ...
    # name = 'test'
    result = os.path.join(SIM_DIR, name) + ".pkl"
...
def hide_self_and_descendants(obj, hide=True, keyframes=False, frame=None):
    ...
    if keyframes == True:
        if frame == None:
            raise Exception(
                "in hide_self_and_descendants(), frame must be "
...
def circle_grid(
    ...
    for i in range(num_rings):
        ...
        for j in range(num_dots):
            ...
            loc = [radius * math.cos(angle), radius * math.sin(angle), 0]
...
def make_parent_tree():
    ...
    while len(intersections) > 0:
        ...
        for mesh in meshes:
            ...
            if (
                len(mesh.children) == 0
                and mesh.parent == None
                and "Cylinder" not in mesh.name
                and score > max_score
            ):
                # score < min_score:
                # min_score = score
...
def import_object(filename, *folders, **kwargs):
    ...
    if "wiggle" in kwargs:
        ...
        wiggle = False
    if (
        filename == "boerd_blob" or filename == "boerd_blob_squat" or filename == "blob"
    ) and wiggle == True:
        ...
        for t in range(wiggle_cycle_length):
            if t == 0:
                ...
                bone.keyframe_insert(
                    data_path="rotation_quaternion", frame=t * wiggle_slow_factor
                )
                ...
                timing_factor = (
                    (wind_down_time - time_left) * time_left / wind_down_time ** 2
                )
    ...
    if filename == "stanford_bunny":
        ...
        try:
            eye_fcurve = (
                ref_obj.children[0]
                .children[0]
                .animation_data.action.fcurves.find("scale", index=1)
            )
...
def cam_and_swivel(
    ...
    cam_bobj = bobject.Bobject(
        location=cam_location, rotation_euler=cam_rotation_euler, name=cam_name
    )
...
def make_animations_linear(
    thing_with_animation_data, data_paths=None, extrapolate=False
):
    if data_paths == None:
...
def fade(object=None, start_time=0, duration_time=1, fade_out=True, extent=1):
...
def highlight_object(object=None, start_time=0, duration_time=1):
    start_frame = start_time * FRAME_RATE
    # end_frame = start_frame + duration_time * FRAME_RATE
    if object == None:
        raise Warning("Need object to fade")
    ...
    for slot in object.material_slots:
        ...
        color.keyframe_insert(
            ...
            frame=start_frame + duration_time / 4 * FRAME_RATE,
        )
        color.keyframe_insert(
            data_path="default_value",
            frame=start_frame + 3 * duration_time / 4 * FRAME_RATE,
        )
        color.default_value = original_color
        color.keyframe_insert(
            data_path="default_value", frame=start_frame + duration_time * FRAME_RATE
        )


def end_rigid_body(obj=None, end_frame=None):
    print(obj.name)
...
def finish_noise(error=False):
    ...
    # windound.Beep(freq, duration)"""
    if error == True:
        # windound.PlaySound("SystemHand", # windound.SND_ALIAS)
        pass
    else:
        pass
        # windound.MessageBeep()
